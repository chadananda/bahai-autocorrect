...
var _autoCorrectBahai = function(str, stripTags) {
  ...
  let wordChar = 'a-záíúA-ZÁÍÚ'
  let anyWord = `([${wordChar}]*)`
  let startWord = `([^${wordChar}])`
  let endWord = `(?![${wordChar}])`
  CommonMispellings.forEach(function(item) {
    if (item.split('=').length == 2) {
      ...
      str = str.replace(new RegExp(startWord + find + endWord, 'g'), repl)
      str = str.replace(new RegExp(startWord + findUpper + endWord, 'g'), repl.toUpperCase())
